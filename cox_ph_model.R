...
HR_Cox_table <- function(DF = DF,                      # complete data frame
  ...
  if (!is.null(weights)) {
    ...
    if (variations == "multi") {
      #multivariate HR and p
      HR_multi <- coxph(as.formula(paste(survival_analysis,
                                         "~",
      ...
      for(i in 1:length(variables)){
        HR_uni <- rbind(HR_uni,
                        coxph(as.formula(paste(survival_analysis,
                                               "~",
                                               variables[i],
                                               sep = "")),
                              data = DF,
                              weights = weights)%>%fit2df(estimate_suffix = "_0.95CI"))
      }
      HR_title <- finalfit(DF,
                           dependent = survival_analysis,
                           explanatory = variables)%>%
